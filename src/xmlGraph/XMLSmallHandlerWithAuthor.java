...
class edgeLabelUnitWithAuthor{
	int labelID;
	double weight;
	String labelName;
	String labelValue;
	boolean labelHave;
	edgeLabelUnitWithAuthor(){
		labelHave = false;
		weight = 1.0;
	}
};
public class XMLSmallHandlerWithAuthor extends DefaultHandler{

	private String COMMONFILE = "data/common_words.txt";
	private String DERICTION = "data/DBLPSmallWithAuthor/";
	...
	private HashMap<Object, edgeLabelUnitWithAuthor> attributeMap = null;
	...
	public void startDocument() throws SAXException{
		...
		try {
			...
			attributeMap = new HashMap<Object, edgeLabelUnitWithAuthor>();
	...
	throws SAXException {
		...
		if(!qName.equalsIgnoreCase("dblp")){
			if(!attributeMap.containsKey(qName.toLowerCase())){
				edgeLabelUnitWithAuthor newLabel = new edgeLabelUnitWithAuthor();
	...
	throws SAXException {
		Iterator<Entry<Object, edgeLabelUnitWithAuthor>> iter = attributeMap.entrySet().iterator();
		while(iter.hasNext()){
			@SuppressWarnings("rawtypes")
			Map.Entry entryBMap = (Map.Entry) iter.next(); 
			String label = (String)entryBMap.getKey(); 
			edgeLabelUnitWithAuthor labelC = attributeMap.get(label);
