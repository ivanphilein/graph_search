...
class edgeLabelUnitNoRoot1{
	int labelID;
	double weight;
	String labelName;
	String labelValue;
	boolean labelHave;
	edgeLabelUnitNoRoot1(){
		labelHave = false;
		weight = 1.0;
	}
};
public class CopyOfXMLHandlerNoRoot extends DefaultHandler{

	private String folder = "data/DBLPHugeNoRoot/";
	...
	private String NODEFORCHECK = folder+"nodefrocheck.txt";
	...
	private HashMap<Object, edgeLabelUnitNoRoot1> edgeLabelIDMap = null;
	...
	public void startDocument() throws SAXException{
		...
		try {
			...
			edgeLabelIDMap = new HashMap<Object, edgeLabelUnitNoRoot1>();
	...
	throws SAXException {
		...
		if(!qName.equalsIgnoreCase("dblp")){
			if(!edgeLabelIDMap.containsKey(qName)){
				edgeLabelUnitNoRoot1 newLabel = new edgeLabelUnitNoRoot1();
				...
				try {
					...
					if(outputStr!=null){
						...
						if(outputStr!=null)
							...
							writeFile(outError,"qName: "+qName+" keyStr: "+keyStr+"nodeID :"+nodeID);
						}
						//end of clean part
						//for clean part
	...
	throws SAXException {
		haveEdge = true;
		Iterator<Entry<Object, edgeLabelUnitNoRoot1>> iter = edgeLabelIDMap.entrySet().iterator();
		while(iter.hasNext()){
			@SuppressWarnings("rawtypes")
			Map.Entry entryBMap = (Map.Entry) iter.next(); 
			String label = (String)entryBMap.getKey(); 
			edgeLabelUnitNoRoot1 labelC = edgeLabelIDMap.get(label);
